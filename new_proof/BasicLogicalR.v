...
Proof.
  ...
  - econstructor; auto. apply basicR_typable_empty in H3. basic_typing_solver2.
Qed.
...
Ltac basic_typing_solver2 :=
  (repeat match goal with
     ...
     | [|- _ ⊢t (vbiop _) ⋮v _ ] => constructor; listctx_set_solver
...
Proof.
  ...
  - rewrite msubst_tlete. auto_exists_L; simpl; intros.
    msubst_preserves_typing_tac.
  - rewrite msubst_tletbiop. econstructor; eauto.
    eapply closed_has_type_under_any_ctx_value; op_simpl; eauto; listctx_set_solver.
    auto_exists_L_intros.
    msubst_preserves_typing_tac.
...
(* Inductive ty_arrow_argRz: ty -> ty -> ty -> Prop := *)
(* | ty_arrow_argR_basez: forall Tx (B: base_ty), ty_arrow_argR (Tx ⤍ B) Tx B *)
(* | ty_arrow_argR_arrowz: forall Tx T1 T2, *)
(*     (forall T', ty_arrow_argR (Tx ⤍ T') Tx T') -> *)
(*     ty_arrow_argR (Tx ⤍ T1 ⤍ T2) Tx (T1 ⤍ T2). *)

Inductive err_trans : tm -> tm -> Prop :=
| err_trans_tlete1: forall e, err_trans (tlete terr e) terr
| err_trans_tlete2: forall e1 e2 e, err_trans e1 e2 -> err_trans (tlete e1 e) (tlete e2 e).

Lemma err_trans_basicR_preserve: forall T e1 e2, err_trans e1 e2 -> basicR T e1 -> basicR T e2.
Proof.
  induction T; intros. admit. split. admit.
  intros. apply IHT2 with (mk_app e1 e0). admit. admit.

Lemma terr_basicR: forall T, basicR T terr.
Proof.
  induction T. admit. split. admit.
...
Proof.
  ...
  apply (tm_has_type_mutual_rec
    ...
    repeat match goal with
      ...
      | [|- _ ⊢t (vbiop _) ⋮v _] => constructor; auto
  ...
  - rewrite msubst_vlam.
    eapply multistep_preserves_basicR'. 

    unfold mk_app. destruct T0.
