...
(*     invclear H7. constructor; auto. *)
(*     + invclear H2. constructor; auto. invclear H7. constructor; auto. *)
(*       unfold not_fv_in_refinement. intros. apply H. intros. invclear H8. *)
(*     + invclear H3. constructor; auto. *)
(*     + simpl in H4. simpl. set_solver. *)
(*   - mydestr. invclear H2; auto_ty_exfalso. *)
(*     + constructor; auto; fold (@app (atom * rty)). listctx_set_simpl. *)
...
Proof.
  ...
  rewrite bstate_insert_commute; auto.
  rewrite H2; auto.
Qed.
...
Proof.
  ...
  - destruct H; destruct H0; mydestr; subst; auto.
    + destruct (decide (a = x0)); auto; subst.
      assert ((mk_op_ret op ^r^ x1) = [v:ret_ty_of_op op|0| ∅|refinement_open 0 x1
                                    (λ (bst : bstate) (_ : state) (v : constant), eval_op op (bst 0) (bst 1) v)]); auto.
      rewrite H. apply state_subst_implies_rty_subst_eq2_backward; auto.
      simpl in H2. dec_solver2.
    + destruct (decide (a = x1)); auto; subst.
      unfold mk_op_ret. rewrite rty_open_commute; auto.
      unfold mk_op_ret in H1. rewrite rty_open_commute in H1; auto.
      unfold mk_op_ret in H2. rewrite rty_open_commute in H2; auto.
      apply state_subst_implies_rty_subst_eq2_backward; auto.
      simpl; auto. simpl; auto. dec_solver1.
      simpl in H2. dec_solver2.
    + destruct (decide (a = x0)); auto; subst.
      assert ((⅋{<[x0:=c_x]> st}⟦τ⟧{Γ ++ [(x, {1 ~r> {x0 := c_x }v x0} (mk_op_ret op ^r^ x1))] }) e).
      { assert ({x0 := c_x }v x0 = c_x). simpl. dec_solver2.
        rewrite H. rewrite H in H2. clear H.
        unfold mk_op_ret in H2. rewrite rty_open_commute in H2; auto.
        assert (ok_dctx (dom aset (<[x0:=c_x]> st)) (Γ ++ [(x, {1 ~r> c_x} (mk_op_ret op ^r^ x1))])).
        apply state_subst_implies_rty_subst_eq2_forward_ok_dctx with (a:=x0) (c_x :=c_x) in H1; eauto.
        simpl in H1. rewrite decide_True in H1; auto. unfold refinement_set_open. apply tletbiop_aux1; auto.
        rewrite dom_insert_simp. set_solver. admit.
        (* - assert ((2 `min` 0) `min` 1 = 0) by lia. rewrite H. clear H. *)
        (*   rewrite ok_dctx_post_destruct in H1; mydestr. invclear H3. invclear H4. invclear H7. *)
        (*   unfold bound_in_refinement. intros. unfold bound_in_refinement in H4. *)
        (*   specialize (H4 _ _ st0 v H7). admit. *)
        (* -  *)
        unfold mk_op_ret in H. rewrite rty_open_commute in H; auto.
        unfold mk_op_ret. rewrite rty_open_commute; auto.
      ...
      apply state_subst_implies_rty_subst_eq2_backward; auto.
      assert ({a := c_x }v x0 = x0). simpl. dec_solver1; auto.
      rewrite H in H2. clear H.
      unfold mk_op_ret in H2. rewrite rty_open_commute in H2; auto.
      unfold mk_op_ret in H1. rewrite rty_open_commute in H1; auto.
      unfold mk_op_ret. rewrite rty_open_commute; auto.
      apply state_subst_implies_rty_subst_eq2_backward; auto.
      simpl; auto. dec_solver1.
Admitted.
...
Proof.
  ...
  - constructor; auto. denotation_simp. apply tletbiop_typable_ctx_dummy_cons; auto.
    ...
    assert ((⅋{<[a:=c_x]> st}⟦τ⟧{Γ ++ [(x, {1 ~r> ({a := c_x }v v2)} (mk_op_ret op ^r^ ({a := c_x }v v1)))] }) ({a := c_x }t tletbiop op v1 v2 e)).
    { eapply IHΓ; eauto; fold value_subst.
      rewrite fv_of_subst_tm_closed; refinement_solver. admit.
      fold tm_subst. rewrite <- subst_open_var_tm; auto. admit.
      denotation_simp. }
    admit.
  - constructor; auto. denotation_simp. apply tletbiop_typable_ctx_dummy_cons; auto.
    mydestr. auto_meet_exists Hmeet. auto_meet_reduce. auto_under v_x. simpl.
  ...
  - constructor; auto. denotation_simp. apply tletbiop_typable_ctx_dummy_cons; auto.
    ...
    destruct H3; destruct H4; mydestr; subst.
    + eapply IHΓ; eauto.
      rewrite fv_of_subst_tm_closed; refinement_solver.
      rewrite <- subst_open_var_tm; auto. denotation_simp. basic_typing_solver.
    + simpl. destruct (decide (a = x0)).
      eapply IHΓ; eauto.

      eapply state_subst_implies_rty_subst_eq2_forward; auto. admit.
      apply inv_ctxrR_regular in H15; mydestr; auto.
      eapply IHΓ; eauto.
      rewrite fv_of_subst_tm_closed; refinement_solver.
      rewrite <- subst_open_var_tm; auto. denotation_simp. basic_typing_solver.

    eapply has_basety_implies_constant_or_var; eauto. apply H20.
    Check state_subst_implies_rty_subst_eq2.
    rewrite state_subst_implies_rty_subst_eq2.
