...
(*[Axiom]This follows from the term_order_trans so can be axiomatized*)
...
(* Axiom eta_lam_inner_subst:  
forall tm1 tm2, tm1 <=< tm2 -> vlam a T tm1 <=< vlam a T (tm2)*)

(*directly from Axiomn eta_lam_inner_subst*)
...
(*[Axiom] next two can directly go to Axioms*)
...
(*Axiomatize :: 
This follows from the transitivity, synmmetry and Lemmas eta_application_const_to_lete_const and 
Lemma eta_reduction
Axiom eta1 ...
*)
...
Admitted.
(* Can we have an axiom  forall v: value. tlete a e_x v <=< v ?? This will obviate the need for many Lemmas like the following*)
Lemma eta_subst_in_const: forall a e_x (c: constant), (tlete a e_x c) <=< c.
Admitted.

(*A similar axiom as we define for <=< in eta_lam_inner_subst can be defined for <-< 
\forall tm1 <-< tm2, tlete x = tm1 in e <-< tlete x = tm2 in e2, this will remove following lemma.*)
Lemma eta_snd_let_reduce: forall x1 e1 x2 e2 x (c2: constant),
    e2 -->* c2 ->
    (tlete x1 e1 (tlete x2 c2 (tletapp x x1 x2 x))) <-< (tlete x1 e1 (tlete x2 e2 (tletapp x x1 x2 x))).
Admitted.

(* This can be axiomatized as directly follows from the definition of <=<, i.e.
, forall tm1 tm2, tm1 <=< tm2 -> tm1 <-< tm2 /\ tm2 <-< tm1
Thus from Axiom eta1 we get the following three Lemma, eta4, eta5 and eta6 *)
...
(* There must be a way to define a generic meta Lemma relating the Lemmas for tletapp and ltetbinop,
by defining a binop in terms of tletapp.
however this is not feasible as of now, Is it correct that tletbiop is just a syntactic sugar over tletapp ??
If true, we can reduce many of the Lemmas which follow (8, op, 9, 10) as a case of tletapp, for which we have 
simialr Lemmas/Axioms defined above. *)
...
(*follows directly from lete_preserve_not_free in CoreLangSimp.v*)
...
(* I am afraid, we will have to keep all these Lemmas for fixpoint*)
