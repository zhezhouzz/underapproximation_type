...
Proof with eauto.
  ...
  (* op *)
  - intros op a b (Hinv & Hclosed) Hab st. apply tmR_in_ctx_pre_weakening with (Gamma1 := Gamma) (Gamma2 := l_empty); auto.
    rewrite app_nil_r...
  (* var over base *)
  - intros T x Hinv (phi & Hfind) st. apply l_find_right_most_some_spec in Hfind.
  ...
  (* var under base *)
  - intros T x (Hinv & Hclosed) (phi & Hfind) st. apply l_find_right_most_some_spec in Hfind.
    destruct Hfind as (Gamma1 & Gamma2 & Hctxconcat & HGamma2). subst.
  ...
  (* var under arrarr *)
  - intros x t1 t2 (Hinv & Hclosed) Hfind st. apply l_find_right_most_some_spec in Hfind.
    destruct Hfind as (Gamma1 & Gamma2 & Hctxconcat & HGamma2). subst.
  ...
  (* var under oarr *)
  - intros x a tau_a tau_b (Hinv & Hclosed) Hfind st. apply l_find_right_most_some_spec in Hfind.
    destruct Hfind as (Gamma1 & Gamma2 & Hctxconcat & HGamma2). subst.
    eapply tmR_in_ctx_post_weakening with (Gamma1:= Gamma1 ++ ((x, Uty (a o: tau_a o--> tau_b))::nil)) (Gamma2 := Gamma2)... rewrite app_assoc...
    + eapply ctx_inv_implies_mem_fresh_and_close...
    + apply under_variable_has_same_type_in_ctx...
  (* lam *)
  - intros x tau_x e tau (Hinv & Hclosed) He HeD st.
  ...
  - intros x tau_x e tau Hwf He HeD st.
    assert (well_formed (Gamma <l> x :l: tau_x) tau) as HH. apply type_judgement_implies_inv in He...
  ...
  (* eq *)
  - intros e tau1 tau2 (Hinv & Hclosed) He HeD Hsub1 Hsub2 st.
  ...
  (* matchb true *)
  - intros v e1 e2 tau (Hinv & Hclosed) Hv HvD He1 He1D st. eapply tmR_in_ctx_preserve_matchb_true...
  - intros v e1 e2 tau (Hinv & Hclosed) Hv HvD He1 He1D st. eapply tmR_in_ctx_preserve_matchb_false...
Qed.
...
Proof with eauto.
  intros.
  apply soundness in H. remember (H empstate) as HH.
  inversion HH; subst.
